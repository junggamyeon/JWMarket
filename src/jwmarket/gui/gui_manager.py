...
class GuiManager:
    ...
    def _show_all_listings(self, player: Player, page: int = 1) -> None:
        async def task():
            try:
                listings = await self._plugin.auction_service.get_active_listings(page=page, per_page=45)
                def callback():
                    if not listings:
                    ...
                    try:
                        from endstone_inventoryui import Menu, MenuType, UIInventory
                        sym = self._plugin.economy_api.currency_symbol
                        menu = Menu(MenuType.DOUBLE_CHEST, self._plugin.message_formatter.format("gui_ah_page_title", page=page))
                        inventory = menu.inventory
                        
                        for i, listing in enumerate(listings):
                            item_stack = self._create_item_from_data(listing.item_data, override_amount=listing.item_amount)
                            try:
                                meta = item_stack.item_meta
                                price_str = self._plugin.message_formatter.format_price(listing.price, sym)
                                meta.lore = [
                                    self._plugin.message_formatter.format("gui_item_lore_price", price=price_str),
                                    self._plugin.message_formatter.format("gui_item_lore_seller", seller=listing.seller_name),
                                    self._plugin.message_formatter.format("gui_item_lore_click")
                                ]
                                item_stack.set_item_meta(meta)
                            except Exception:
                                pass
                            inventory.set_item(i, item_stack)

                        if page > 1:
                            prev_item = ItemStack("minecraft:arrow", 1)
                            try:
                                pm = prev_item.item_meta
                                pm.display_name = self._plugin.message_formatter.format("gui_btn_prev_page")
                                prev_item.set_item_meta(pm)
                            except Exception: pass
                            inventory.set_item(45, prev_item)
                            
                        if len(listings) >= 45:
                            next_item = ItemStack("minecraft:arrow", 1)
                            try:
                                nm = next_item.item_meta
                                nm.display_name = self._plugin.message_formatter.format("gui_btn_next_page")
                                next_item.set_item_meta(nm)
                            except Exception: pass
                            inventory.set_item(53, next_item)

                        def on_click(p: Player, slot: int, item: ItemStack, inv: UIInventory):
                            menu.close(p)
                            if slot < len(listings):
                                self._show_listing_detail(p, listings[slot].id)
                            elif slot == 45 and page > 1:
                                self._show_all_listings(p, page - 1)
                            elif slot == 53 and len(listings) >= 45:
                                self._show_all_listings(p, page + 1)
                            else:
                        ...
                        for listing in listings:
                            price_str = self._plugin.message_formatter.format_price(listing.price, sym)
                            btn_text = self._plugin.message_formatter.format("gui_btn_listing", item_name=listing.item_display, amount=listing.item_amount, price=price_str, seller=listing.seller_name)
    ...
    def _show_category_listings(self, player: Player, category: str, page: int = 1) -> None:
        async def task():
            try:
                listings = await self._plugin.auction_service.get_listings_by_category(category, page=page, per_page=45)
                def callback():
                    if not listings:
                    ...
                    try:
                        from endstone_inventoryui import Menu, MenuType, UIInventory
                        sym = self._plugin.economy_api.currency_symbol
                        menu = Menu(MenuType.DOUBLE_CHEST, self._plugin.message_formatter.format("gui_cat_title", category=category.title(), page=page))
                        inventory = menu.inventory
                        ...
                        def on_click(p: Player, slot: int, item: ItemStack, inv: UIInventory):
                            ...
                            if slot < len(listings):
                                ...
                                self._show_category_listings(p, category, page - 1)
                            elif slot == 53 and len(listings) >= 45:
                                self._show_category_listings(p, category, page + 1)
                            else:
                                self.show_main_menu(p)
                        ...
                        for listing in listings:
                            price_str = self._plugin.message_formatter.format_price(listing.price, sym)
                            btn_text = self._plugin.message_formatter.format("gui_btn_listing_no_seller", item_name=listing.item_display, amount=listing.item_amount, price=price_str)
                            form.add_button(btn_text, on_click=lambda p, lid=listing.id: self._show_listing_detail(p, lid))
                        if len(listings) >= 45:
                            form.add_button(self._plugin.message_formatter.format("gui_btn_next_page"), on_click=lambda p: self._show_category_listings(p, category, page + 1))
                        if page > 1:
                            form.add_button(self._plugin.message_formatter.format("gui_btn_prev_page"), on_click=lambda p: self._show_category_listings(p, category, page - 1))
                        form.add_button(self._plugin.message_formatter.format("gui_btn_back"), on_click=lambda p: self.show_main_menu(p))
                        player.send_form(form)
                self._plugin.server.scheduler.run_task(self._plugin, callback)
            except Exception as e:
            ...
            try:
                ...
                def callback():
                    if result:
                        ...
                        player.send_message(self._plugin.message_formatter.format("gui_listing_cancel_failed"))
                self._plugin.server.scheduler.run_task(self._plugin, callback)
            except Exception as e:
            ...
            try:
                ...
                def callback():
                    ...
                    for listing in results:
                        price_str = self._plugin.message_formatter.format_price(listing.price, sym)
                        btn = self._plugin.message_formatter.format("gui_btn_listing", item_name=listing.item_display, amount=listing.item_amount, price=price_str, seller=listing.seller_name)
        ...
        async def task():
            try:
                ...
                def callback():
                    ...
                    for listing in listings:
                        price_str = self._plugin.message_formatter.format_price(listing.price, sym)
                        btn = self._plugin.message_formatter.format("gui_btn_listing_no_seller", item_name=listing.item_display, amount=listing.item_amount, price=price_str)
                        form.add_button(btn, on_click=lambda p, lid=listing.id: self._show_listing_detail(p, lid))
                    form.add_button(self._plugin.message_formatter.format("gui_btn_back"), on_click=lambda p: self.show_main_menu(p))
                    player.send_form(form)
                self._plugin.server.scheduler.run_task(self._plugin, callback)
            ...
            try:
                ...
                def callback():
                    ...
                    for order in orders:
                        ...
                        btn = self._plugin.message_formatter.format("gui_btn_order", item_name=order.item_display, filled=filled, price=price_str)
            ...
            try:
                ...
                def callback():
                    if result:
                        ...
                        player.send_message(self._plugin.message_formatter.format("gui_order_cancel_failed"))
                self._plugin.server.scheduler.run_task(self._plugin, callback)
            except Exception as e:
    ...
    def show_browse_orders(self, player: Player) -> None:
        form = ActionForm(
            title=self._plugin.message_formatter.format("gui_my_orders_title"),
            content=self._plugin.message_formatter.format("gui_main_menu_content"),
        )
        categories = self._plugin.search_service.get_category_list()
        form.add_button(self._plugin.message_formatter.format("gui_btn_all_listings"), icon="textures/items/emerald", on_click=lambda p: self._show_all_orders(p))
        for cat in categories:
            form.add_button(
                cat["display_name"],
                icon=cat["icon"] if cat["icon"] else None,
                on_click=lambda p, c=cat["key"]: self._show_category_orders(p, c),
            )
        form.add_button(self._plugin.message_formatter.format("gui_btn_back"), on_click=lambda p: self.show_main_menu(p))
        player.send_form(form)

    def _show_all_orders(self, player: Player, page: int = 1) -> None:
        async def task():
            try:
                orders = await self._plugin.order_service.browse_orders(page=page, per_page=45)
                def callback():
                    if not orders:
                    ...
                    try:
                        from endstone_inventoryui import Menu, MenuType, UIInventory
                        sym = self._plugin.economy_api.currency_symbol
                        menu = Menu(MenuType.DOUBLE_CHEST, self._plugin.message_formatter.format("gui_orders_page_title", page=page))
                        inventory = menu.inventory
                        
                        for i, order in enumerate(orders):
                            # It's an order, let's create a generic item stack of order.item_type
                            # Orders only store item_type
                            try:
                                item_stack = ItemStack(order.item_type, 1)
                            except Exception:
                                item_stack = ItemStack("minecraft:paper", 1)
                            ...
                            try:
                                meta = item_stack.item_meta
                                meta.display_name = self._plugin.message_formatter.format("gui_order_item_name", item_name=order.item_display, remaining=remaining)
                                meta.lore = [
                                    self._plugin.message_formatter.format("gui_order_lore_price", price=price_str),
                                    self._plugin.message_formatter.format("gui_order_lore_buyer", buyer=order.creator_name),
                                    self._plugin.message_formatter.format("gui_order_lore_click")
                                ]
                                item_stack.set_item_meta(meta)
                            except Exception:
                                pass
                        ...
                        def on_click(p: Player, slot: int, item: ItemStack, inv: UIInventory):
                            menu.close(p)
                            if slot < len(orders):
                                self._show_fulfill_detail(p, orders[slot].id)
                            elif slot == 45 and page > 1:
                                self._show_all_orders(p, page - 1)
                            elif slot == 53 and len(orders) >= 45:
                                self._show_all_orders(p, page + 1)
                            else:
                                self.show_browse_orders(p)
                                
                        menu.set_listener(on_click)
                        menu.send_to(player)
                    except ImportError:
                        sym = self._plugin.economy_api.currency_symbol
                        form = ActionForm(title=self._plugin.message_formatter.format("gui_my_orders_title"), content=self._plugin.message_formatter.format("gui_orders_page_content", page=page))
                        for order in orders:
                            price_str = self._plugin.message_formatter.format_price(order.price_each, sym)
                            remaining = order.quantity_total - order.quantity_filled
                            btn = self._plugin.message_formatter.format("gui_btn_order_browse", item_name=order.item_display, remaining=remaining, price=price_str, buyer=order.creator_name)
                            form.add_button(btn, on_click=lambda p, oid=order.id: self._show_fulfill_detail(p, oid))
                        if len(orders) >= 45:
                            form.add_button(self._plugin.message_formatter.format("gui_btn_next_page"), on_click=lambda p: self._show_all_orders(p, page + 1))
                        if page > 1:
                            form.add_button(self._plugin.message_formatter.format("gui_btn_prev_page"), on_click=lambda p: self._show_all_orders(p, page - 1))
    ...
    def _show_category_orders(self, player: Player, category: str, page: int = 1) -> None:
        async def task():
            try:
                orders = await self._plugin.order_service.browse_orders(category=category, page=page, per_page=45)
                def callback():
                    if not orders:
                        form = ActionForm(title=self._plugin.message_formatter.format("gui_my_orders_title"), content=self._plugin.message_formatter.format("gui_ah_empty"))
                        form.add_button(self._plugin.message_formatter.format("gui_btn_back"), on_click=lambda p: self.show_browse_orders(p))
                    ...
                    try:
                        ...
                        def on_click(p: Player, slot: int, item: ItemStack, inv: UIInventory):
                            ...
                            if slot < len(orders):
                                ...
                                self._show_category_orders(p, category, page - 1)
                            elif slot == 53 and len(orders) >= 45:
                                self._show_category_orders(p, category, page + 1)
                            else:
                                self.show_browse_orders(p)
                        ...
                        for order in orders:
                            ...
                            form.add_button(btn, on_click=lambda p, oid=order.id: self._show_fulfill_detail(p, oid))
                        if len(orders) >= 45:
                            form.add_button(self._plugin.message_formatter.format("gui_btn_next_page"), on_click=lambda p: self._show_category_orders(p, category, page + 1))
                        if page > 1:
                            form.add_button(self._plugin.message_formatter.format("gui_btn_prev_page"), on_click=lambda p: self._show_category_orders(p, category, page - 1))
                        form.add_button(self._plugin.message_formatter.format("gui_btn_back"), on_click=lambda p: self.show_browse_orders(p))
                        player.send_form(form)
                self._plugin.server.scheduler.run_task(self._plugin, callback)
            except Exception as e:
    ...
    def _fulfill_order_prompt(self, player: Player, order_id: int, max_qty: int) -> None:
        ...
        async def task():
            try:
                ...
                def callback():
                    if not result.success:
                        if result.error == "creator_insufficient_funds":
                            player.send_message(self._plugin.message_formatter.format("gui_order_creator_funds"))
                        else:
                    ...
                    if result.creator_name:
                        ...
                        if order_creator:
                            try:
                                ...
                                item_display = self._plugin.item_serializer.get_display_name(result.item_type)
                                order_creator.send_message(self._plugin.message_formatter.format("order_fulfilled_creator", item_name=item_display, amount=result.quantity_filled))
                            except Exception as e:

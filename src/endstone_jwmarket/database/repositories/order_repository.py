...
class OrderRepository:
    ...
    async def create_order(
        ...
        cursor = await self._db.execute(
            """
            INSERT INTO orders (creator_uuid, creator_name, order_type, item_type, item_display, quantity_total, quantity_filled, price_each, escrow_amount, expires_at)
            VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, datetime('now', '+' || ? || ' hours'))
            """,
            (creator_uuid, creator_name, order_type, item_type, item_display, quantity, price_each, escrow_amount, duration_hours),
        )
    ...
    async def browse_orders(self, limit: int = 10, offset: int = 0) -> list[OrderRecord]:
    ...
    def _map_row(self, row: Any) -> OrderRecord:
        return OrderRecord(
            ...
            price_each=row["price_each"], escrow_amount=row["escrow_amount"],
